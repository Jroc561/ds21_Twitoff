...
def predict_user(user0_name, user1_name, hypo_tweet_text):
    ...
    # vertically stacking (combining vectors) to train model
    vects = np.vstack([user0_vects, user1_vects])

    # generate labels for the vects array
    labels = np.concatenate([np.zeros(len(user0.tweets)), np.ones(len(user1.tweets))])

    # train model and instantiate
    log_reg = LogisticRegression().fit(vects, labels)

    # using nlp model to generate embeddings - vectorize_tweet()
    hypo_tweet_vect = vectorize_tweet(hypo_tweet_text).reshape(1,-1)

    # predicts and returns 0 or 1 depending upon Logistic Regression models predictions
